...
class Bot:
    ...
    def sing_up(self):
        ...
        self.session.post('https://zakupki.mos.ru/auth/realms/PpRealm/login-actions/authenticate?'
                          ...
                          'client_id=PpApp&'
                          'tab_id=N_BMcQ2LesA', json={'username': self.login, 'password': 'agent03062002A'})
...
def work_function_1(authorization, auction_url, min_price):
    bot_object = Bot(authorization, 'musheeg@yandex.ru', auction_url, min_price)
    func = bot_object.work_constant
    while True:
        if not func():
            return f'Бот ИП Аванесов закончил работу'
